...
const FIND_BY_ID_SQL = 'SELECT username, first_name, last_name FROM user WHERE id = ?'
...
class UserGateway {
    ...
    async create(firstName, lastName, username, password) {
        try {
            ...
            try {
                const [rows] = await conn.execute(CREATE_SQL, [username, password, firstName, lastName])
